...
def run(settings):
    ...
    use_torch_compile = getattr(cfg.TRAIN, "TORCH_COMPILE", False)
    if use_torch_compile and settings.local_rank in [-1, 0]:
        try:
    ...
    # Adjust BN momentum for small-batch training
    bn_momentum = getattr(cfg.TRAIN, "BN_MOMENTUM", None)
    if bn_momentum is not None:
        for m in net.modules():
            if isinstance(m, (torch.nn.BatchNorm2d, torch.nn.BatchNorm1d)):
                ...
                if settings.local_rank in [-1, 0]:
                    print(f"  BN momentum adjusted: {m.__class__.__name__} ({old_m} → {bn_momentum})")
