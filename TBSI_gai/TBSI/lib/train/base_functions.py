...
def update_settings(settings, cfg):
    ...
    settings.save_epochs = getattr(cfg.TRAIN, "SAVE_EPOCHS", [])
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        if stage2_baseline:
            ...
            if is_main_process():
                print("Stage 2 fine-tuning: post_fusion_block + box_head + deg_state_token trainable.")
                for n, p in net.named_parameters():
                    if p.requires_grad:
                        print(n)
        else:
            ...
            if temporal_lr:
                param_dicts = [
                    {
                        "params": [p for n, p in net.named_parameters() if "temporal_token" in n and p.requires_grad],
                        "lr": temporal_lr,
                    },
