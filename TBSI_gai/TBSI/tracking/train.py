...
def main():
    args = parse_args()
    python_cmd = sys.executable  # Use the same Python interpreter that invoked this script
    if args.mode == "single":
        train_cmd = "%s lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d " \
                    ...
                    % (python_cmd, args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv,
        ...
        train_cmd = "%s -m torch.distributed.launch --nproc_per_node %d --master_port %d lib/train/run_training.py " \
                    ...
                    % (python_cmd, args.nproc_per_node, random.randint(10000, 50000), args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv, args.use_wandb,
        ...
        train_cmd = "%s -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
                    % (python_cmd, args.nproc_per_node, args.ip, args.port, args.world_size, args.rank, args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv, args.use_wandb,
                       args.distill, args.script_teacher, args.config_teacher)
    else:
