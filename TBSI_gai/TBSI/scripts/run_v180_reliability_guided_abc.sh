...
LOG_DIR="output/diagnostics/reliability_guided_v180"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v1.8.0-reliability-guided-selective-interaction-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v1.8.0 reliability-guided ABC"
python scripts/summarize_v180_reliability_guided.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v1.8.0 reliability-guided selective interaction ABC"
