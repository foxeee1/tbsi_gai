...
LOG_DIR="output/diagnostics/fcc_margin_v412"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.2-fcc-margin-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.2 margin FCC ABC"
python scripts/summarize_v412_fcc_margin.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.2 margin FCC ABC"
