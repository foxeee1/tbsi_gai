...
LOG_DIR="output/diagnostics/directional_fcc_v419"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.9-directional-fcc-middle-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.9 directional FCC ABC"
python scripts/summarize_v419_directional_fcc.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.9 directional FCC ABC"
