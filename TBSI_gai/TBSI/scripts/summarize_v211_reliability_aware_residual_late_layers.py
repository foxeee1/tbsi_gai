...
OUT_DIR = Path("output/diagnostics/reliability_aware_residual_late_layers_v211")
REPORT_NAME = "mini_lasher_test_reliability_aware_residual_late_layers_v211"
...
TRACKER_PARAMS = [
    "v2.1.1-reliability-aware-residual-late-layers-A",
    "v2.1.1-reliability-aware-residual-late-layers-B",
    "v2.1.1-reliability-aware-residual-late-layers-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v2.1.1-reliability-aware-residual-late-layers-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v211_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v211": mean_metrics([overall[f"v2.1.1-reliability-aware-residual-late-layers-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v211_minus_{tag}"] for s in "ABC"}
        mean[f"v211_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v211_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Output residual gate after TBSI bridge interactions should preserve the "
            "v1.6.0 gains while reducing attribute-level drops on baseline-strong "
            "or unreliable-interaction scenes."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v211_compare.json"
