...
LOG_DIR="output/diagnostics/freqgate_v400"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.0.0-freqgate-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.0.0 FreqGATE ABC"
python scripts/summarize_v400_freqgate.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.0.0 FreqGATE selective interaction ABC"
