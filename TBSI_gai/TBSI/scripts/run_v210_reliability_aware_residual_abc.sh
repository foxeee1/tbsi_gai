...
LOG_DIR="output/diagnostics/reliability_aware_residual_v210"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v2.1.0-reliability-aware-residual-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v2.1.0 reliability-aware residual ABC"
python scripts/summarize_v210_reliability_aware_residual.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v2.1.0 reliability-aware residual selective interaction ABC"
