...
LOG_DIR="output/diagnostics/competitive_residual_bridge_v321"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v3.2.1-competitive-residual-bridge-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v3.2.1 competitive residual bridge ABC"
python scripts/summarize_v321_competitive_residual_bridge.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v3.2.1 competitive residual bridge selective interaction ABC"
