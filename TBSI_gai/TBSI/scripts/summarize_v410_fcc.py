...
OUT_DIR = Path("output/diagnostics/fcc_v410")
REPORT_NAME = "mini_lasher_test_fcc_v410"
...
V400_SUMMARY_FILE = Path("output/diagnostics/freqgate_v400/attr_metrics_v400_compare.json")
...
def main():
    ...
    v400_summary = json.load(V400_SUMMARY_FILE.open())
    ...
    overall.update(v400_summary["overall"])
    by_category.update(v400_summary["by_category"])
    ...
    mean = {
        "v410": mean_metrics([overall[f"v4.1.0-fcc-{s}"] for s in "ABC"]),
        ...
        "v400": mean_metrics([overall[REFERENCE_PARAMS["v400"].format(seed=s)] for s in "ABC"]),
    }
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v410_minus_{tag}"] for s in "ABC"}
        mean[f"v410_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v410_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Frequency cross-modal consistency should suppress search tokens with "
            "above-average RGB/TIR spatial-frequency disagreement, improving TBSI "
            "cross-modal interaction without prompt tuning, learned reliability "
            "statistics, or output residual gating."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v410_compare.json"
