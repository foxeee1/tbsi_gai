...
OUT_DIR = Path("output/diagnostics/template_prior_reliability_residual_v212")
REPORT_NAME = "mini_lasher_test_template_prior_reliability_residual_v212"
...
TRACKER_PARAMS = [
    "v2.1.2-template-prior-reliability-residual-A",
    "v2.1.2-template-prior-reliability-residual-B",
    "v2.1.2-template-prior-reliability-residual-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v2.1.2-template-prior-reliability-residual-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v212_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v212": mean_metrics([overall[f"v2.1.2-template-prior-reliability-residual-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v212_minus_{tag}"] for s in "ABC"}
        mean[f"v212_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v212_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    out_path = OUT_DIR / "attr_metrics_v212_compare.json"
