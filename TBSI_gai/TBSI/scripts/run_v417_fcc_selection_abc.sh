...
LOG_DIR="output/diagnostics/fcc_selection_v417"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.7-fcc-selection-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.7 selection FCC ABC"
python scripts/summarize_v417_fcc_selection.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.7 selection FCC ABC"
