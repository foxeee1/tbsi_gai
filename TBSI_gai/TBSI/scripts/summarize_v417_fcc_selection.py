...
OUT_DIR = Path("output/diagnostics/fcc_selection_v417")
REPORT_NAME = "mini_lasher_test_fcc_selection_v417"
...
TRACKER_PARAMS = [
    "v4.1.7-fcc-selection-A",
    "v4.1.7-fcc-selection-B",
    "v4.1.7-fcc-selection-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v4.1.7-fcc-selection-{seed}"
    ...
    mean = {
        "v417": mean_metrics([overall[f"v4.1.7-fcc-selection-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
    ...
    for tag in REFERENCE_PARAMS:
        ...
        mean[f"v417_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v417_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Selection FCC should keep the lowest-disagreement tokens and hard-suppress "
            "the rest of the search tokens."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v417_compare.json"
