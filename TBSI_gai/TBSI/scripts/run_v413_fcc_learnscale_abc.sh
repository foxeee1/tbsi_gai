...
LOG_DIR="output/diagnostics/fcc_learnscale_v413"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.3-fcc-learnscale-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.3 learnable-scale FCC ABC"
python scripts/summarize_v413_fcc_learnscale.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.3 learnable-scale FCC ABC"
