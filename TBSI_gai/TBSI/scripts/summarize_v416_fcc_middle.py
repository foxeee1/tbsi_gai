...
OUT_DIR = Path("output/diagnostics/fcc_middle_v416")
REPORT_NAME = "mini_lasher_test_fcc_middle_v416"
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v4.1.6-fcc-middle-{seed}"
    ...
    mean = {
        "v416": mean_metrics([overall[f"v4.1.6-fcc-middle-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
    ...
    for tag in REFERENCE_PARAMS:
        ...
        mean[f"v416_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v416_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Middle-layer FCC should restrict the same threshold-based frequency "
            "consistency bias to the middle TBSI bridge only."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v416_compare.json"
