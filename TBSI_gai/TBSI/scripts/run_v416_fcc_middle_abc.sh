...
LOG_DIR="output/diagnostics/fcc_middle_v416"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.6-fcc-middle-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.6 middle-layer FCC ABC"
python scripts/summarize_v416_fcc_middle.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.6 middle-layer FCC ABC"
