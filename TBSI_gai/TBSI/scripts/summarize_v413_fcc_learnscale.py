...
OUT_DIR = Path("output/diagnostics/fcc_learnscale_v413")
REPORT_NAME = "mini_lasher_test_fcc_learnscale_v413"
...
V412_SUMMARY_FILE = Path("output/diagnostics/fcc_margin_v412/attr_metrics_v412_compare.json")

TRACKER_PARAMS = [
    "v4.1.3-fcc-learnscale-A",
    "v4.1.3-fcc-learnscale-B",
    "v4.1.3-fcc-learnscale-C",
]
...
def load_learned_scales():
    stats = {}
    for seed in "ABC":
        cfg = f"v4.1.3-fcc-learnscale-{seed}"
        ckpt = Path(f"output/experiments/{cfg}/checkpoints/TBSITrack_ep0015.pth.tar")
        state = torch.load(ckpt, map_location="cpu")["net"]
        scales = {
            k: float(v.item())
            for k, v in state.items()
            if k.endswith("freq_consistency.scale")
        }
        stats[cfg] = scales
    return stats
...
def main():
    ...
    merge_reference(overall, by_category, V412_SUMMARY_FILE)
    ...
    mean = {
        "v413": mean_metrics([overall[f"v4.1.3-fcc-learnscale-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
        mean[tag] = mean_metrics([overall[pattern.format(seed=s)] for s in "ABC"])
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v413_minus_{tag}"] for s in "ABC"}
        mean[f"v413_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v413_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "learned_scales": load_learned_scales(),
        "categories": categories,
        "report_name": REPORT_NAME,
        "hypothesis": (
            "Learnable FCC scale should preserve global frequency consistency while "
            "letting the model reduce over-suppression in sensitive layers."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v413_compare.json"
    json.dump(summary, out_path.open("w", encoding="utf-8"), indent=2, ensure_ascii=False)
    print(json.dumps({"wrote": str(out_path), "mean": mean, "learned_scales": summary["learned_scales"]}, indent=2, ensure_ascii=False))
