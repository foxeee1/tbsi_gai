...
LOG_DIR="output/diagnostics/cfs_reliability_bridge_v310"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v3.1.0-cfs-reliability-bridge-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v3.1.0 cfs reliability bridge ABC"
python scripts/summarize_v310_cfs_reliability_bridge.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v3.1.0 cfs reliability bridge selective interaction ABC"
