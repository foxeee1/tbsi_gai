...
LOG_DIR="output/diagnostics/freqrel_v430"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.3.0-freqrel-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.3.0 FreqRel ABC"
python scripts/summarize_v430_freqrel.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.3.0 FreqRel ABC"
