...
LOG_DIR="output/diagnostics/template_conditioned_bridge_v300"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v3.0.0-template-conditioned-bridge-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v3.0.0 template-conditioned bridge ABC"
python scripts/summarize_v300_template_conditioned_bridge.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v3.0.0 template-conditioned bridge selective interaction ABC"
