...
OUT_DIR = Path("output/diagnostics/directional_fcc_v419")
REPORT_NAME = "mini_lasher_test_directional_fcc_v419"
...
TRACKER_PARAMS = [
    "v4.1.9-directional-fcc-middle-A",
    "v4.1.9-directional-fcc-middle-B",
    "v4.1.9-directional-fcc-middle-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v4.1.9-directional-fcc-middle-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v419_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v419": mean_metrics([overall[f"v4.1.9-directional-fcc-middle-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
        mean[tag] = mean_metrics([overall[pattern.format(seed=s)] for s in "ABC"])
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v419_minus_{tag}"] for s in "ABC"}
        mean[f"v419_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v419_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Directional FCC should preserve RGB/TIR frequency direction and learn "
            "separate modality preferences instead of symmetrically suppressing disagreement."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v419_compare.json"
