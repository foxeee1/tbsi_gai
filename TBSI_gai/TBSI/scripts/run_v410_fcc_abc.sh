...
LOG_DIR="output/diagnostics/fcc_v410"
mkdir -p "${LOG_DIR}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.0 FCC ABC"
python scripts/summarize_v410_fcc.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.0 FCC selective interaction ABC"
