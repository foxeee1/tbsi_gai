...
OUT_DIR = Path("output/diagnostics/template_conditioned_bridge_v300")
REPORT_NAME = "mini_lasher_test_template_conditioned_bridge_v300"
...
TRACKER_PARAMS = [
    "v3.0.0-template-conditioned-bridge-A",
    "v3.0.0-template-conditioned-bridge-B",
    "v3.0.0-template-conditioned-bridge-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v3.0.0-template-conditioned-bridge-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v300_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v300": mean_metrics([overall[f"v3.0.0-template-conditioned-bridge-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v300_minus_{tag}"] for s in "ABC"}
        mean[f"v300_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v300_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    out_path = OUT_DIR / "attr_metrics_v300_compare.json"
