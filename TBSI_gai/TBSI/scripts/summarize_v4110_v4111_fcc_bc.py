...
OUT_DIR = Path("output/diagnostics/fcc_bc_v4110_v4111")
REPORT_NAME = "mini_lasher_test_fcc_bc_v4110_v4111"
...
V420_SUMMARY_FILE = Path("output/diagnostics/fcc_residual_v420/attr_metrics_v420_compare.json")
V416_SUMMARY_FILE = Path("output/diagnostics/fcc_middle_v416/attr_metrics_v416_compare.json")
V417_SUMMARY_FILE = Path("output/diagnostics/fcc_selection_v417/attr_metrics_v417_compare.json")
V418_SUMMARY_FILE = Path("output/diagnostics/fcc_conflict_act_v418/attr_metrics_v418_compare.json")
V419_SUMMARY_FILE = Path("output/diagnostics/directional_fcc_v419/attr_metrics_v419_compare.json")

TRACKER_PARAMS = [
    "v4.1.10-fcc-lowmid-middle-A",
    "v4.1.10-fcc-lowmid-middle-B",
    "v4.1.10-fcc-lowmid-middle-C",
    "v4.1.11-fcc-layerwise-A",
    "v4.1.11-fcc-layerwise-B",
    "v4.1.11-fcc-layerwise-C",
]
...
REFERENCE_PARAMS = {
    ...
    "v416": "v4.1.6-fcc-middle-{seed}",
    "v417": "v4.1.7-fcc-selection-{seed}",
    "v418": "v4.1.8-fcc-conflict-act-{seed}",
    "v419": "v4.1.9-directional-fcc-middle-{seed}",
    "v420": "v4.2.0-fcc-residual-{seed}",
}
...
def main():
    ...
    merge_reference(overall, by_category, V420_SUMMARY_FILE)
    merge_reference(overall, by_category, V416_SUMMARY_FILE)
    merge_reference(overall, by_category, V417_SUMMARY_FILE)
    merge_reference(overall, by_category, V418_SUMMARY_FILE)
    merge_reference(overall, by_category, V419_SUMMARY_FILE)
    ...
    deltas = {}
    variants = {
        "v4110": "v4.1.10-fcc-lowmid-middle",
        "v4111": "v4.1.11-fcc-layerwise",
    }
    for variant_tag, variant in variants.items():
        for seed in "ABC":
            current = f"{variant}-{seed}"
            for tag, pattern in REFERENCE_PARAMS.items():
                ref = pattern.format(seed=seed)
                key = f"{seed}_{variant_tag}_minus_{tag}"
                deltas[key] = {
    ...
    mean = {
        "v4110": mean_metrics([overall[f"v4.1.10-fcc-lowmid-middle-{s}"] for s in "ABC"]),
        "v4111": mean_metrics([overall[f"v4.1.11-fcc-layerwise-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
        mean[tag] = mean_metrics([overall[pattern.format(seed=s)] for s in "ABC"])
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        for variant_tag in ["v4110", "v4111"]:
            seed_deltas = {s: deltas[f"{s}_{variant_tag}_minus_{tag}"] for s in "ABC"}
            mean[f"{variant_tag}_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
            mean_deltas_by_category[f"{variant_tag}_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "v4.1.10 tests middle-layer low/mid-band FCC. v4.1.11 tests "
            "middle-layer conflict suppression plus late-layer consistency boosting."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v4110_v4111_compare.json"
