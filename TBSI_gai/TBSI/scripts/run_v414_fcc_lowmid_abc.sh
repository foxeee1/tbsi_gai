...
LOG_DIR="output/diagnostics/fcc_lowmid_v414"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.4-fcc-lowmid-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.4 low-mid FCC ABC"
python scripts/summarize_v414_fcc_lowmid.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.4 low-mid FCC ABC"
