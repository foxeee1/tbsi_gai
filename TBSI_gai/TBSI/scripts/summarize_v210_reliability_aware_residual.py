...
OUT_DIR = Path("output/diagnostics/reliability_aware_residual_v210")
REPORT_NAME = "mini_lasher_test_reliability_aware_residual_v210"
...
TRACKER_PARAMS = [
    "v2.1.0-reliability-aware-residual-A",
    "v2.1.0-reliability-aware-residual-B",
    "v2.1.0-reliability-aware-residual-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v2.1.0-reliability-aware-residual-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v210_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v210": mean_metrics([overall[f"v2.1.0-reliability-aware-residual-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v210_minus_{tag}"] for s in "ABC"}
        mean[f"v210_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v210_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    out_path = OUT_DIR / "attr_metrics_v210_compare.json"
