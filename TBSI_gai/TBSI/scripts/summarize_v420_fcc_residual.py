...
OUT_DIR = Path("output/diagnostics/fcc_residual_v420")
REPORT_NAME = "mini_lasher_test_fcc_residual_v420"
...
V411_SUMMARY_FILE = Path("output/diagnostics/fcc_v411/attr_metrics_v411_compare.json")
...
def main():
    ...
    merge_reference(overall, by_category, V411_SUMMARY_FILE)
    ...
    mean = {
        "v420": mean_metrics([overall[f"v4.2.0-fcc-residual-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
        mean[tag] = mean_metrics([overall[pattern.format(seed=s)] for s in "ABC"])
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v420_minus_{tag}"] for s in "ABC"}
        mean[f"v420_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v420_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Global FCC and reliability residual operate in different spaces; "
            "if complementary, v4.2.0 should exceed both v4.1.0 and v2.1.0."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v420_compare.json"
