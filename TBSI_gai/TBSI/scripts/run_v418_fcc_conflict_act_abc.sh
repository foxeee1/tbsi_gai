...
LOG_DIR="output/diagnostics/fcc_conflict_act_v418"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.8-fcc-conflict-act-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.8 conflict-activated FCC ABC"
python scripts/summarize_v418_fcc_conflict_act.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.8 conflict-activated FCC ABC"
