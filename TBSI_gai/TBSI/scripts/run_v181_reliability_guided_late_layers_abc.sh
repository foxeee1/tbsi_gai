...
LOG_DIR="output/diagnostics/reliability_guided_v181"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v1.8.1-reliability-guided-late-layers-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v1.8.1 reliability-guided late-layers ABC"
python scripts/summarize_v181_reliability_guided_late_layers.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v1.8.1 reliability-guided late-layers selective interaction ABC"
