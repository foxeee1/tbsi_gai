...
TRACKER_PARAMS = [
    ...
    "v1.8.0-reliability-guided-selective-interaction-B",
    "v1.8.0-reliability-guided-selective-interaction-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v1.8.0-reliability-guided-selective-interaction-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v180_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v180": mean_metrics([overall[f"v1.8.0-reliability-guided-selective-interaction-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v180_minus_{tag}"] for s in "ABC"}
        mean[f"v180_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v180_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Reliability-guided residual signal decoupling should preserve the "
            "v1.6.0 gains while reducing attribute-level drops on baseline-strong "
            "or unreliable-interaction scenes."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v180_compare.json"
