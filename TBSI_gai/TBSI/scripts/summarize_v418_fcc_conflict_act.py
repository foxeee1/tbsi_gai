...
OUT_DIR = Path("output/diagnostics/fcc_conflict_act_v418")
REPORT_NAME = "mini_lasher_test_fcc_conflict_act_v418"
...
TRACKER_PARAMS = [
    "v4.1.8-fcc-conflict-act-A",
    "v4.1.8-fcc-conflict-act-B",
    "v4.1.8-fcc-conflict-act-C",
]
...
def main():
    ...
    mean = {
        "v418": mean_metrics([overall[f"v4.1.8-fcc-conflict-act-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
        mean[tag] = mean_metrics([overall[pattern.format(seed=s)] for s in "ABC"])
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v418_minus_{tag}"] for s in "ABC"}
        mean[f"v418_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v418_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Conflict-activated FCC should keep middle-layer threshold FCC active "
            "only when the frame contains concentrated cross-modal frequency disagreement."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v418_compare.json"
