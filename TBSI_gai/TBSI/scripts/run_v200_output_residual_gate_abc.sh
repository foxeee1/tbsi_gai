...
LOG_DIR="output/diagnostics/output_residual_gate_v200"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v2.0.0-output-residual-gate-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v2.0.0 output-residual-gate ABC"
python scripts/summarize_v200_output_residual_gate.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v2.0.0 output-residual-gate selective interaction ABC"
