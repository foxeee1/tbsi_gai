...
OUT_DIR = Path("output/diagnostics/competitive_bridge_v320")
REPORT_NAME = "mini_lasher_test_competitive_bridge_v320"
...
TRACKER_PARAMS = [
    "v3.2.0-competitive-bridge-A",
    "v3.2.0-competitive-bridge-B",
    "v3.2.0-competitive-bridge-C",
]
...
def main():
    ...
    mean = {
        "v320": mean_metrics([overall[f"v3.2.0-competitive-bridge-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v320_minus_{tag}"] for s in "ABC"}
        mean[f"v320_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v320_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Competitive RGB/TIR search-to-template bridge fusion should allocate "
    ...
    out_path = OUT_DIR / "attr_metrics_v320_compare.json"
