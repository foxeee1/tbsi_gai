...
LOG_DIR="output/diagnostics/fcc_residual_v420"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.2.0-fcc-residual-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.2.0 FCC + residual ABC"
python scripts/summarize_v420_fcc_residual.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.2.0 FCC + residual ABC"
