...
OUT_DIR = Path("output/diagnostics/fcc_margin_v412")
REPORT_NAME = "mini_lasher_test_fcc_margin_v412"
...
def main():
    ...
    mean = {
        "v412": mean_metrics([overall[f"v4.1.2-fcc-margin-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
        mean[tag] = mean_metrics([overall[pattern.format(seed=s)] for s in "ABC"])
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v412_minus_{tag}"] for s in "ABC"}
        mean[f"v412_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v412_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Margin FCC should keep the global frequency-consistency mechanism "
            "but suppress only stronger-than-average cross-modal conflicts."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v412_compare.json"
