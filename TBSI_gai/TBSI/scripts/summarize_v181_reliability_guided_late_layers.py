...
OUT_DIR = Path("output/diagnostics/reliability_guided_v181")
REPORT_NAME = "mini_lasher_test_reliability_guided_v181"
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v1.8.1-reliability-guided-late-layers-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v181_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v181": mean_metrics([overall[f"v1.8.1-reliability-guided-late-layers-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v181_minus_{tag}"] for s in "ABC"}
        mean[f"v181_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v181_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Reliability-guided residual signal decoupling on late TBSI bridge layers should preserve the "
            "v1.6.0 gains while reducing attribute-level drops on baseline-strong "
            "or unreliable-interaction scenes."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v181_compare.json"
