...
LOG_DIR="output/diagnostics/clipped_template_conditioned_bridge_v302"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v3.0.2-clipped-template-conditioned-bridge-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v3.0.2 clipped template-conditioned bridge ABC"
python scripts/summarize_v302_clipped_template_conditioned_bridge.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v3.0.2 clipped template-conditioned bridge selective interaction ABC"
