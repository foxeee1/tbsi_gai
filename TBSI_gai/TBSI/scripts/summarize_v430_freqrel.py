...
OUT_DIR = Path("output/diagnostics/freqrel_v430")
REPORT_NAME = "mini_lasher_test_freqrel_v430"
...
def main():
    ...
    categories = json.load(CATEGORY_FILE.open(encoding="utf-8"))["categories"]
    overall = {}
    by_category = {}
    ...
    mean = {
        "v430": mean_metrics([overall[f"v4.3.0-freqrel-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v430_minus_{tag}"] for s in "ABC"}
        mean[f"v430_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v430_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "FreqRel replaces fixed scalar FCC with a learnable frequency-relation "
            "estimator while keeping the intervention limited to s2t attention bias."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v430_compare.json"
