...
LOG_DIR="output/diagnostics/competitive_bridge_v320"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v3.2.0-competitive-bridge-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v3.2.0 competitive bridge ABC"
python scripts/summarize_v320_competitive_bridge.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v3.2.0 competitive bridge selective interaction ABC"
