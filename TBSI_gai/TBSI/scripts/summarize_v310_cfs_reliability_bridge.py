...
OUT_DIR = Path("output/diagnostics/cfs_reliability_bridge_v310")
REPORT_NAME = "mini_lasher_test_cfs_reliability_bridge_v310"
...
TRACKER_PARAMS = [
    "v3.1.0-cfs-reliability-bridge-A",
    "v3.1.0-cfs-reliability-bridge-B",
    "v3.1.0-cfs-reliability-bridge-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v3.1.0-cfs-reliability-bridge-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v310_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v310": mean_metrics([overall[f"v3.1.0-cfs-reliability-bridge-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v310_minus_{tag}"] for s in "ABC"}
        mean[f"v310_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v310_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    out_path = OUT_DIR / "attr_metrics_v310_compare.json"
