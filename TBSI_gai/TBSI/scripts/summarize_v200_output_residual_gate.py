...
OUT_DIR = Path("output/diagnostics/output_residual_gate_v200")
REPORT_NAME = "mini_lasher_test_output_residual_gate_v200"
...
TRACKER_PARAMS = [
    "v2.0.0-output-residual-gate-A",
    "v2.0.0-output-residual-gate-B",
    "v2.0.0-output-residual-gate-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v2.0.0-output-residual-gate-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v200_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v200": mean_metrics([overall[f"v2.0.0-output-residual-gate-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v200_minus_{tag}"] for s in "ABC"}
        mean[f"v200_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v200_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    out_path = OUT_DIR / "attr_metrics_v200_compare.json"
