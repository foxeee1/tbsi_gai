...
OUT_DIR = Path("output/diagnostics/fcc_lowmid_v414")
REPORT_NAME = "mini_lasher_test_fcc_lowmid_v414"
...
V413_SUMMARY_FILE = Path("output/diagnostics/fcc_learnscale_v413/attr_metrics_v413_compare.json")
...
REFERENCE_PARAMS = {
    ...
    "v413": "v4.1.3-fcc-learnscale-{seed}",
...
def main():
    ...
    for path in [
        ...
        merge_reference(overall, by_category, path)
    ...
    mean = {
        "v414": mean_metrics([overall[f"v4.1.4-fcc-lowmid-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
        mean[tag] = mean_metrics([overall[pattern.format(seed=s)] for s in "ABC"])
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v414_minus_{tag}"] for s in "ABC"}
        mean[f"v414_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v414_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Low-mid FCC should use more frequency structure than low-only FCC, "
            "preserving ThermalCross gains while improving general conflict coverage."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v414_compare.json"
