...
LOG_DIR="output/diagnostics/template_prior_reliability_residual_v212"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v2.1.2-template-prior-reliability-residual-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v2.1.2 template-prior reliability residual ABC"
python scripts/summarize_v212_template_prior_reliability_residual.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v2.1.2 template-prior reliability residual selective interaction ABC"
