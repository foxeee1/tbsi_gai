...
LOG_DIR="output/diagnostics/fcc_threshold_v415"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.5-fcc-threshold-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.5 threshold FCC ABC"
python scripts/summarize_v415_fcc_threshold.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.5 threshold FCC ABC"
