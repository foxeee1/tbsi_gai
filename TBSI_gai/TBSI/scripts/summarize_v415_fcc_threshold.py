...
OUT_DIR = Path("output/diagnostics/fcc_threshold_v415")
REPORT_NAME = "mini_lasher_test_fcc_threshold_v415"
...
TRACKER_PARAMS = [
    "v4.1.5-fcc-threshold-A",
    "v4.1.5-fcc-threshold-B",
    "v4.1.5-fcc-threshold-C",
]
...
def main():
    ...
    mean = {
        "v415": mean_metrics([overall[f"v4.1.5-fcc-threshold-{s}"] for s in "ABC"]),
    }
    for tag, pattern in REFERENCE_PARAMS.items():
        mean[tag] = mean_metrics([overall[pattern.format(seed=s)] for s in "ABC"])
    mean_deltas_by_category = {}
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v415_minus_{tag}"] for s in "ABC"}
        mean[f"v415_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v415_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Threshold FCC should replace zscore normalization with an absolute "
            "frequency-disagreement threshold and a learnable bias scale."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v415_compare.json"
