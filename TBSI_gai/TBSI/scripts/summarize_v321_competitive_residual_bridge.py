...
OUT_DIR = Path("output/diagnostics/competitive_residual_bridge_v321")
REPORT_NAME = "mini_lasher_test_competitive_residual_bridge_v321"
...
TRACKER_PARAMS = [
    "v3.2.1-competitive-residual-bridge-A",
    "v3.2.1-competitive-residual-bridge-B",
    "v3.2.1-competitive-residual-bridge-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v3.2.1-competitive-residual-bridge-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v321_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v321": mean_metrics([overall[f"v3.2.1-competitive-residual-bridge-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v321_minus_{tag}"] for s in "ABC"}
        mean[f"v321_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v321_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    summary = {
        ...
        "hypothesis": (
            "Conservative competitive RGB/TIR search-to-template bridge fusion should preserve the original sequential TBSI bridge while allocating "
            "the shared template update to the modality with stronger interaction "
            "evidence, improving attribute coverage without hand-crafted reliability "
            "statistics or output residual gating."
        ),
    }
    out_path = OUT_DIR / "attr_metrics_v321_compare.json"
