...
OUT_DIR = Path("output/diagnostics/positive_template_conditioned_bridge_v301")
REPORT_NAME = "mini_lasher_test_positive_template_conditioned_bridge_v301"
...
TRACKER_PARAMS = [
    "v3.0.1-positive-template-conditioned-bridge-A",
    "v3.0.1-positive-template-conditioned-bridge-B",
    "v3.0.1-positive-template-conditioned-bridge-C",
]
...
def main():
    ...
    deltas = {}
    for seed in "ABC":
        current = f"v3.0.1-positive-template-conditioned-bridge-{seed}"
        for tag, pattern in REFERENCE_PARAMS.items():
            ref = pattern.format(seed=seed)
            key = f"{seed}_v301_minus_{tag}"
            deltas[key] = {
    ...
    mean = {
        "v301": mean_metrics([overall[f"v3.0.1-positive-template-conditioned-bridge-{s}"] for s in "ABC"]),
    ...
    for tag in REFERENCE_PARAMS:
        seed_deltas = {s: deltas[f"{s}_v301_minus_{tag}"] for s in "ABC"}
        mean[f"v301_minus_{tag}"] = mean_metrics([d["overall"] for d in seed_deltas.values()])
        mean_deltas_by_category[f"v301_minus_{tag}"] = category_delta_mean(seed_deltas)
    ...
    out_path = OUT_DIR / "attr_metrics_v301_compare.json"
