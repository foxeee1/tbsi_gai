...
LOG_DIR="output/diagnostics/fcc_v411"
mkdir -p "${LOG_DIR}"
...
run_one() {
  local subset="$1"
  local cfg="v4.1.1-fcc-local-${subset}"
...
echo "[$(date '+%F %T')] SUMMARIZE v4.1.1 local FCC ABC"
python scripts/summarize_v411_fcc.py \
  > "${LOG_DIR}/summarize.console.log" 2>&1

echo "[$(date '+%F %T')] ALL DONE v4.1.1 local FCC ABC"
